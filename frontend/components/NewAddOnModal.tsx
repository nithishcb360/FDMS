...
import { serviceAddonsApi } from '@/lib/api/service-addons';
...
export default function NewAddOnModal({ isOpen, onClose, onAddOnCreated }: NewAddOnModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Service add-on created successfully!');
      onAddOnCreated?.();
      onClose();
    } catch (error) {
      console.error('Error creating add-on:', error);
      alert('Failed to create service add-on. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4">
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">New Service Add-on</h2>
              <p className="text-sm text-gray-600">Add a new service or product</p>
            </div>
          </div>
        </div>
