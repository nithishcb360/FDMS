...
export default function DynamicSidebar() {
  ...
  const [allowedTabs, setAllowedTabs] = useState<TabData[]>([]);
  const [loading, setLoading] = useState(true);
  const pathname = usePathname();
  const router = useRouter();

  // Fetch user data and their allowed tabs
  useEffect(() => {
    const fetchUserAndTabs = async () => {
      try {
        ...
        // Fetch tabs for user's role
        if (userData.role_id) {
          const tabs = await rolesApi.getRoleTabs(userData.role_id, token);
          setAllowedTabs(tabs.filter(tab => tab.is_enabled));
        } else if (userData.is_superuser) {
          // Superusers get all tabs - we'll need to fetch all tabs
          // For now, show all hardcoded tabs
          setAllowedTabs([]);
        }
      } catch (error: any) {
  ...
  const isTabAllowed = (tabName: string) => {
    ...
    if (allowedTabs.length === 0) return false; // No tabs assigned
    return allowedTabs.some(tab => tab.name === tabName);
  };

  const isSectionAllowed = (sectionName: string) => {
    if (user?.is_superuser) return true;
    // Check if any tab in this section is allowed
    return allowedTabs.some(tab => tab.parent === sectionName || tab.name === sectionName);
  };
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-slate-700 via-slate-800 to-slate-900 text-white transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl flex flex-col`}>
        {/* Header */}
        ...
        <nav className="py-2 flex-1 overflow-y-auto">
          ...
          {/* Services - Only show if section is allowed */}
          {isSectionAllowed('services') && (
            <div>
              ...
              {expandedSections.includes('services') && (
                <div className="bg-slate-800/50 py-1">
                  {isTabAllowed('service_schedules') && (
                    <a href="/services/schedules" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/services/schedules') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                  ...
                  {isTabAllowed('arrangements') && (
                    <a href="/services/arrangements" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/services/arrangements') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                  ...
                  {isTabAllowed('venue_bookings') && (
                    <a href="/services/venue-bookings" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/services/venue-bookings') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                  ...
                  {isTabAllowed('service_addons') && (
                    <a href="/services/add-ons" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/services/add-ons') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                    ...
                    <>
                      ...
                      {expandedSections.includes('inventory') && (
                        <div className="bg-slate-900/50">
                          {isTabAllowed('inventory_products') && (
                          ...
                          {isTabAllowed('inventory_categories') && (
                          ...
                          {isTabAllowed('inventory_suppliers') && (
                          ...
                          {isTabAllowed('inventory_purchase_orders') && (
                          ...
                          {isTabAllowed('inventory_stock_movements') && (
                    ...
                    <>
                      ...
                      {expandedSections.includes('fleet') && (
                        <div className="bg-slate-900/50">
                          ...
                          {isTabAllowed('fleet_assignments') && (
                          ...
                          {isTabAllowed('fleet_maintenance') && (
            ...
            <div>
              ...
              {expandedSections.includes('crm') && (
                <div className="bg-slate-800/50 py-1">
                  ...
                  {isTabAllowed('crm_communications') && (
          ...
          {isSectionAllowed('documents') && (
            <div>
              ...
              {expandedSections.includes('documents') && (
                <div className="bg-slate-800/50 py-1">
                  {isTabAllowed('all_documents') && (
                    <a href="/all-documents" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/all-documents') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                      <svg className="w-3 h-3 mr-2" fill="currentColor" viewBox="0 0 24 24"><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>
                      All Documents
                    </a>
                  )}
                  {isTabAllowed('document_types') && (
                  ...
                  {isTabAllowed('templates') && (
                    <a href="/templates" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/templates') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
          ...
          {isSectionAllowed('settings') && (
            <div>
              ...
              {expandedSections.includes('settings') && (
                <div className="bg-slate-800/50 py-1">
                  {isTabAllowed('settings_branches') && (
                    <a href="/settings/branches" className={`flex items-center px-4 py-2 pl-11 text-sm ${isActive('/settings/branches') ? 'bg-yellow-500 text-slate-900' : 'hover:bg-slate-700 text-white'}`}>
                      <svg className="w-3 h-3 mr-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10z"/></svg>
                      Branches
                    </a>
                  )}
                  {isTabAllowed('settings_service_types') && (
                  ...
                  {isTabAllowed('settings_service_packages') && (
                  ...
                  {isTabAllowed('settings_venue_types') && (
                  ...
                  {isTabAllowed('settings_tax_codes') && (
                  ...
                  {isTabAllowed('settings_payment_modes') && (
                  ...
                  {isTabAllowed('settings_religious_rites') && (
                  ...
                  {isTabAllowed('settings_document_types') && (
                  ...
                  {isTabAllowed('settings_expense_categories') && (
                  ...
                  {isTabAllowed('settings_roles_permissions') && (
                  ...
                  {isTabAllowed('settings_user_management') && (
                  ...
                  {isTabAllowed('settings_system_settings') && (
          ...
          {/* Show message when no tabs are assigned */}
          {!loading && !user?.is_superuser && allowedTabs.length === 0 && (
