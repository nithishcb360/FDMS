...
import { documentTypesApi, DocumentTypeData } from '@/lib/api/document-types';

interface EditDocumentTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  documentType: DocumentTypeData | null;
  onDocumentTypeUpdated?: () => void;
}

const STATUSES = ['Active', 'Inactive'];

export default function EditDocumentTypeModal({ isOpen, onClose, documentType, onDocumentTypeUpdated }: EditDocumentTypeModalProps) {
  ...
  useEffect(() => {
    if (isOpen && documentType) {
      ...
      setFormData({
        ...
        max_size_mb: documentType.max_size_mb?.toString() || '',
        retention_years: documentType.retention_years?.toString() || '',
        require_signature: documentType.require_signature || false,
        require_approval: documentType.require_approval || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await documentTypesApi.update(documentType.id, {
        name: formData.name,
      ...
      alert(error.response?.data?.detail || 'Failed to update document type');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Update document type details</p>
            </div>
            <button
              onClick={onClose}
