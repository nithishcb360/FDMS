...
import { venueBookingsApi, VenueBookingData } from '@/lib/api/venue-bookings';
...
export default function EditBookingModal({ isOpen, onClose, bookingData, onBookingUpdated }: EditBookingModalProps) {
  ...
  useEffect(() => {
    if (isOpen && bookingData) {
      ...
      setFormData({
        case_id: bookingData.case_id.toString(),
        venue: bookingData.venue,
        booking_date: formatDateForInput(bookingData.booking_date),
        booking_time: bookingData.booking_time || '',
        duration_hours: bookingData.duration_hours?.toString() || '2',
        contact_person: bookingData.contact_person || '',
        contact_phone: bookingData.contact_phone || '',
        contact_email: bookingData.contact_email || '',
        cost: bookingData.cost?.toString() || '',
        status: bookingData.status || 'Tentative',
        special_requirements: bookingData.special_requirements || '',
        setup_notes: bookingData.setup_notes || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await venueBookingsApi.update(bookingData.id!, {
      ...
      alert('Venue booking updated successfully!');
      onBookingUpdated?.();
      onClose();
    } catch (error) {
      console.error('Error updating booking:', error);
      alert('Failed to update venue booking. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4">
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">Edit Venue Booking</h2>
              <p className="text-sm text-gray-600">Update venue reservation information</p>
            </div>
          </div>
        </div>
