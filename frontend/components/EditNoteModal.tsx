...
interface EditNoteModalProps {
  ...
  onClose: () => void;
  onSave?: () => void;
  noteData: CaseNoteData | null;
}

export default function EditNoteModal({ isOpen, onClose, onSave, noteData }: EditNoteModalProps) {
  ...
  useEffect(() => {
    if (isOpen && noteData) {
      setFormData({
        case_number: noteData.case_number,
        note_type: noteData.note_type,
        content: noteData.content,
        requires_follow_up: noteData.requires_follow_up,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const updateData: Partial<CaseNoteData> = {
      ...
      await caseNotesApi.update(noteData.id, updateData);
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Edit Case Note</h2>
              <p className="text-sm text-gray-600">Update the note information</p>
            </div>
          </div>
        </div>
